...
fn validate_layer_records(
    ...
    if records_without_id != expected_records_without_id || ids != expected_ids {
        return Err(ExportError::OutputValidation(format!(
            "encoded layer IDs differ: expected {:?}, got {:?} (records without ID: {}, expected {})",
            expected_ids, ids, records_without_id, expected_records_without_id
        )));
    }
    Ok(())
}
