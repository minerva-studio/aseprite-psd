...
导出命令支持 `--compression rle|zip` 和 `--empty-layers include|omit`；省略压缩时默认使用 Photoshop 兼容的 RLE。ZIP 使用无预测 ZIP，仅用于诊断，不属于 Photoshop 兼容目标。省略空图层选项时默认不导出空像素图层。`omit` 按帧过滤没有 cel、cel 不透明度为 0，或 RGBA 像素 alpha 全为 0 的像素层，并递归裁掉空组；仅因隐藏但仍有非透明像素的图层会保留。`include` 保留完整的空/透明状态布局。
